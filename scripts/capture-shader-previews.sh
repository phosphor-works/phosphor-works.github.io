...
# Prefer wf-recorder (native Wayland); fall back to
# gpu-screen-recorder (works on KDE/Wayland, hardware-encodes on
# supported GPUs).
...
capture_one() {
    ...
    # Record the preview rect.
    local raw="${TMP_DIR}/${id}.mp4"
    case "${RECORDER}" in
        wf-recorder)
            # -g "XxY+W+H" geometry; -r framerate; -x mp4 container.
            ...
            ;;
        gpu-screen-recorder)
            # Records the whole screen then crops later with ffmpeg.
    ...
    # Crop + scale + re-encode to VP9 webm.  -crf 34 is a good
    # size/quality knob for subtle-motion shaders; bump to 30 for
    # busier effects (spectrum-bloom, spectrum-pulse).
    ffmpeg -y -nostdin -loglevel error \
        -i "${raw}" \
        -vf "crop=${PREVIEW_W}:${PREVIEW_H}:${PREVIEW_X}:${PREVIEW_Y},scale=${OUT_W}:${OUT_H}:flags=lanczos" \
